...
MEMORY_CLASSIFY_SYSTEM = SystemMessage(
    content=(
        ...
        "Decision rules:\n"
        "  1. Classify as follow_up / elaboration / clarification / correction "
        ...
        "  2. Classify as new_topic ONLY when the subject itself has genuinely "
        "changed (different domain, unrelated concept, no meaningful thematic link "
        "to any recent episode).\n"
        "  3. For any non-new_topic turn you MUST cite the most relevant episode_id "
        "in PARENT:.\n"
        "  4. Do not invent connections — but never miss obvious thematic continuity.\n\n"
...
SEARCH_REACT_SYSTEM = SystemMessage(
    content=(
        ...
        "Available tools:\n"
        "  web_search_tool      — DuckDuckGo web search. Use for general questions, "
        "current events, software documentation, tutorials, and definitions.\n"
        "  academic_search_tool — Semantic Scholar. Use when the query involves "
        "scientific research, peer-reviewed evidence, medicine, or academic topics.\n\n"
        "Search strategy:\n"
        "  1. Analyse the query to decide if it is general (web) or academic "
        "(academic or both).\n"
        "  2. Start with web_search_tool for most queries.\n"
        "  3. Follow with academic_search_tool when the goal references studies, "
        "research findings, clinical evidence, or scientific mechanisms.\n"
        "  4. Use AT MOST 3 tool calls total. Stop as soon as you have sufficient "
        "evidence to cover the query comprehensively.\n"
        "  5. If a tool returns an error or empty results, try the other tool.\n"
        "  6. Do not repeat the same query twice — reformulate before retrying.\n\n"
        "Output rules:\n"
